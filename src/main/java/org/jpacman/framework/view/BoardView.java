...
public class BoardView extends JPanel {
	...
	/**
	 * Create a new view for the board, given an inspector of the actual board
	 * content.
	 * 
	 * @param board
	 *            Model of the board.
	...
	private Graphics2D createDrawArea() {
		if (drawArea == null) {
			drawArea = (BufferedImage) createImage(windowWidth(), windowHeight());
		}
	 ...
	 * 
	 * @param g
	 *            The graphics to paint the board on.
	 */
	@Override
	 ...
	 * 
	 * @param g2
	 *            The graphics to draw the cells on.
	 */
	...
	private Rectangle centeredArea(int startx, int starty, int radius) {
		...
		Point loc = new Point(startx + CELL_WIDTH / 2 - radius, starty + CELL_HEIGHT / 2 - radius);
	...
	private Image spriteImage(Sprite sprite) {
		...
		if (imageLoader != null && sprite != null) {
			if (sprite instanceof Player) {
				img = imageLoader.player(((Player) sprite).getDirection(), animationCount);
			}
	...
	/**
	 * Increment the animation counter, and redisplay, so that the next animation
	 * becomes visible.
	 */
	public void nextAnimation() {
		if (imageLoader != null) {
			animationCount = (animationCount + 1)
					% (imageLoader.monsterAnimationCount() * imageLoader.playerAnimationCount());
			repaint();
		}
	}
